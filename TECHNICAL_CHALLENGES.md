...
1. [다중 환경 관리의 복잡성](#2-다중-환경-관리의-복잡성)
1. [이미지 처리 성능 병목](#3-이미지-처리-성능-병목)
1. [PLC 통신 추상화](#4-plc-통신-추상화)
1. [실시간 데이터 동기화](#5-실시간-데이터-동기화)
1. [학습 및 성장](#6-학습-및-성장)
...
1. **NAT/방화벽** — 일정 시간 통신 없으면 중간 장비가 연결 강제 종료
1. **브라우저 백그라운드 정책** — 백그라운드 탭의 타이머 throttling → Heartbeat 누락
1. **재연결 전략 부재** — 즉시 재시도 → 서버에 동시 요청 폭증
...
1. **Kafka 토픽 자동 분리**: 개발 `_dev` 접미사 자동 부여 → 개발/프로덕션 메시지 혼입 방지
